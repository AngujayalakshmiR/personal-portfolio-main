            ...
            >
              {achievements.map((achievement, index) => (
                <AchievementCard 
                  key={index} 
                  {...achievement} 
                  onImageClick={() => setSelectedImage(achievement.icon)}
                />
              ))}
            </div>
  ...
  return (
    <div className="achievement-card flex-shrink-0 flex flex-col md:w-[400px] w-[320px] justify-around px-6 py-4 rounded-[20px] md:mr-10 mr-6 my-5 transition-colors duration-300 transform border hover:border-transparent dark:border-gray-700 dark:hover:border-transparent">
      <img
        src={props.icon}
        alt={props.event}
        className="w-full md:w-[368px] h-[160px] md:h-[220px] object-contain rounded-lg ring-2 ring-gray-300 mb-4 bg-white cursor-pointer hover:opacity-90 transition-opacity"
